...
export const CircuitDetails: React.FC = () => {
  ...
  return (
    <Card className="border-[#009FE3]/30">
      <Card.Header title={race.track?.name || "Track Details"} subtitle="Event Information" />
      <Card.Content>
        <div className="flex flex-col md:flex-row gap-6 items-start">
          <img
            className="w-full md:w-60 h-60 object-cover rounded-lg border border-[#009FE3]/20"
            src={race.track?.image || "https://static.vecteezy.com/system/resources/previews/006/526/923/original/unknown-location-concepts-vector.jpg"}
            alt="Track Layout"
          />

          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-gray-300 text-sm w-full">
            <Detail label="Date" value={race.date ? new Date(race.date).toLocaleDateString("en-GB", {
                            day: "numeric",
                            month: "long",
                            year: "numeric",
                            hour: "2-digit",
            ...
            <Detail label="Location" value={race.track?.location || "-"} />
            <Detail label="Track Length" value={race.track?.length || "-"} />
            <Detail label="Race Format" value={race.raceFormat || "-"} />
